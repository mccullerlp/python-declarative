        ...
        def __setattr__(self, name, item):
            if isinstance(item, PropertyTransforming):
                if not hasattr(self.__class__, name):
                    raise RuntimeError("Cannot directly assign sub-elements, use ...{cname}.my.{name} = ... instead".format(cname = self.name_child, name = name))
...
class Element(
    ...
    def generate_child_ooa_params(self, name, child):
        return self.ooa_params[name]

    @mproperty_check
    def ooa_params(self, arg = NOARG):
        if arg is NOARG:
            arg = self.parent.generate_child_ooa_params(self.name_child, self)
        return arg
...
class RootElement(Element):
    ...
    @mproperty
    def ooa_params(self, arg = NOARG):
        #if it is a view (as generated by ooa_shadow), then extract only the new elements
    ...
    def regenerate(self, ooa_params = None, **kwargs):
        usekwargs = dict(self._overridable_object_kwargs)
        if ooa_params is not None:
            usekwargs['ooa_params'] = ooa_params
